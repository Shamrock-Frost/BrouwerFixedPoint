...
begin
  ...
  { dsimp [e, limits.is_colimit.cocone_point_unique_up_to_iso],
    apply hT.hom_ext, intros i, cases i;
    simp only [cofork.app_one_eq_π, cofork.is_colimit.π_desc,
               cokernel_cofork.π_of_π, cofork.is_colimit.π_desc_assoc,
               coequalizer.cofork_π, cokernel.π_desc,
               cokernel_cofork.π_eq_zero, zero_comp] },
  rw this,
  split,
...
{ hom := coproduct_homology_comparison _ _ _ _ _,
  ...
  hom_inv_id' := begin
    ...
    { dsimp [t], apply homology.hom_to_ext,
      simp only [category.assoc, homology.lift_ι, kernel.lift_ι_assoc,
                 homology.π'_ι] },
    { rw ht, clear ht, clear t,
