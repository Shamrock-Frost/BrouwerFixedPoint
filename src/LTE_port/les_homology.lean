...
{ app := λ A, cokernel.map _ _ (𝟙 _) ((A.cycles i).arrow)
  ...
  begin
    ...
    simp only [hom.sq_from_right, hom.sq_to_right, homology.π'_map_assoc],
    delta homology.π',
    simp only [homology_iso_cokernel_lift, homology_iso_cokernel_image_to_kernel',
               category.assoc, iso.trans_inv, iso.symm_inv, cokernel_epi_comp_hom,
               cokernel_iso_of_eq_inv_comp_desc, cokernel.π_desc_assoc,
               cokernel.π_desc, kernel_subobject_arrow'_assoc, kernel.lift_ι_assoc],
  end }
.
